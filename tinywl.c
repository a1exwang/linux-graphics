...
void clear_screen(int fd) {
  char buffer[16] = {0};
  int size = sprintf(buffer, "\x1B[2J");
  write(fd, buffer, size);
}

int write_rgb(char *buffer, int r, int g, int b) {
  return sprintf(buffer, "\x1B[;48;2;%d;%d;%dm ", r, g, b);
}
...
static void render_surface(struct wlr_surface *surface,
	...
	unsigned char *pixel_data = malloc(surface->current.height*surface->current.width*4);

	uint32_t flags = 0;
	int w = surface->current.width, h = surface->current.height;
    wlr_renderer_read_pixels(rdata->renderer,
                             WL_SHM_FORMAT_ARGB8888,
                             NULL,
                             4*w, w, h,
                             0, 0, 0, 0, pixel_data);
    char *console_buffer = malloc(60*61*21);
    size_t offset = 0;
    int size = sprintf(console_buffer + offset, "\x1B[2J");
    offset += size;
    clear_screen(tty_fd);
    for (int i = 0; i < 20; i++) {
      for (int j = 0; j < 20; j++) {
        int r = pixel_data[i*w*4 + j*4 + 0];
        int g = pixel_data[i*w*4 + j*4 + 1];
        int b = pixel_data[i*w*4 + j*4 + 2];
//        printf("rgb %d, %d, %d\n", r, g, b);
        size = write_rgb(console_buffer+offset, r, g, b);
        offset += size;
      }
      console_buffer[offset++] = '\n';
    }
    write(tty_fd, console_buffer, offset);
    free(console_buffer);
    free(pixel_data);
